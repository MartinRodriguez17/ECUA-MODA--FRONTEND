  ...
  Future<void> _confirmarAccionUsuario(
    ...
    if (accionTipo == 'suspendido') {
      ...
      if (dias < 1 || dias > 30) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Los días deben ser entre 1 y 30 🛑'),
            backgroundColor: Colors.red,
          ),
        );
        return;
    ...
    try {
      await _adminService.cambiarEstadoUsuario(
        ...
        dias: accionTipo == 'suspendido'
            ? int.tryParse(diasController.text.trim())
            : null,
      );
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('$nombre: $accionTipo ✅'),
          backgroundColor: Colors.black,
        ),
      );
      _cargarUsuarios();
